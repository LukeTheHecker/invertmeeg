...
def get_experiments(n_dipoles: int, quick: bool = False, round2: bool = False, round3: bool = False) -> list[ExperimentConfig]:
    ...
    if quick:
        ...
        experiments.append(ExperimentConfig(label="batch_reps=10", batch_repetitions=10))
        experiments.append(ExperimentConfig(label="epochs=500", epochs=500, patience=500))
    ...
    if round2:
        ...
        experiments.append(ExperimentConfig(
            label="bs1024+ep500",
            batch_size=1024, epochs=500, patience=500,
        ))
        experiments.append(ExperimentConfig(
            label="bs1024+ep500+pat150",
            batch_size=1024, epochs=500, patience=150,
        ))
        experiments.append(ExperimentConfig(
            label="bs2048+ep500",
            batch_size=2048, epochs=500, patience=500,
        ))
        experiments.append(ExperimentConfig(
            label="bs2048+ep300",
            batch_size=2048, epochs=300, patience=150,
        ))
        experiments.append(ExperimentConfig(
            label="bs1024+ep1000",
            batch_size=1024, epochs=1000, patience=300,
        ))
        experiments.append(ExperimentConfig(
            label="bs2048+ep500+u500",
            batch_size=2048, epochs=500, patience=300, n_dense_units=500,
        ))
        experiments.append(ExperimentConfig(
            label="bs4096+ep300",
            batch_size=4096, epochs=300, patience=150,
        ))
        return experiments
    ...
    if round3:
        ...
        experiments.append(ExperimentConfig(
            ...
            batch_size=4096, epochs=500, patience=300, n_dense_units=500,
        ))
        experiments.append(ExperimentConfig(
            label="bs8192+ep500+u500",
            batch_size=8192, epochs=500, patience=300, n_dense_units=500,
        ))
        experiments.append(ExperimentConfig(
            label="bs16384+ep500+u500",
            batch_size=16384, epochs=500, patience=300, n_dense_units=500,
        ))

        # -- More epochs with large data --
        experiments.append(ExperimentConfig(
            label="bs4096+ep1000+u500",
            batch_size=4096, epochs=1000, patience=400, n_dense_units=500,
        ))
        experiments.append(ExperimentConfig(
            label="bs8192+ep1000+u500",
            batch_size=8192, epochs=1000, patience=400, n_dense_units=500,
        ))

        # -- Wider network with massive data --
        experiments.append(ExperimentConfig(
            label="bs8192+ep500+u800",
            batch_size=8192, epochs=500, patience=300, n_dense_units=800,
        ))
        experiments.append(ExperimentConfig(
            label="bs8192+ep500+u1000",
            batch_size=8192, epochs=500, patience=300, n_dense_units=1000,
        ))
...
def train_and_evaluate(
    ...
    logger.info("  units=%d, layers=%d, epochs=%d, lr=%g, patience=%d",
                exp.n_dense_units, exp.n_dense_layers, exp.epochs,
                exp.learning_rate, exp.patience)
    logger.info("  batch_size=%d, batch_reps=%d, temp=%g",
                bs, exp.batch_repetitions, exp.temperature)
    logger.info("=" * 60)
...
def main():
    ...
    parser.add_argument("--quick", action="store_true", help="Run reduced experiment set")
    parser.add_argument("--round2", action="store_true", help="Run round-2 experiments (combine winners)")
    parser.add_argument("--round3", action="store_true", help="Run round-3 experiments (push data limits)")
    parser.add_argument("--only", nargs="*", help="Run only experiments whose labels match these prefixes")
    ...
    # Get experiments
    experiments = get_experiments(n_dipoles, quick=args.quick, round2=args.round2, round3=args.round3)

    # Filter if --only specified
    if args.only:
        experiments = [e for e in experiments if any(e.label.startswith(p) for p in args.only)]
        if not experiments:
