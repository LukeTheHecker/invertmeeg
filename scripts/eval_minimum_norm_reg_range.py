"""Evaluate minimum-norm regularization range behavior across all datasets.

Focuses on minimum-norm solvers that actually search over multiple
regularization candidates. For each dataset and solver, runs two conditions:
  (a) noise_cov = None
...
# Evaluate all minimum_norm and LORETA-family solvers registered in the runner.
# This covers anything we might have missed in earlier targeted runs.
SOLVERS = resolve_solvers(categories=["minimum_norm", "loreta"])
...
def main() -> None:
    print("=" * 80)
    print("Minimum-Norm Regularization Range Evaluation")
    print("=" * 80)
    print(
    ...
    out_path = out_dir / "minimum_norm_reg_range_eval.json"
